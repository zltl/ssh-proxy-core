...
static int test_hmac_sha256_rfc4231(void)
{
    ...
    char hex_out[65];
    int rc = health_check_hmac_sha256(key, 4, data, 28,
                                      hex_out, sizeof(hex_out));
    ASSERT_EQ(rc, 0);
...
static int test_hmac_sha256_empty_data(void)
{
    const char *key = "key";
    char hex_out[65];
    int rc = health_check_hmac_sha256(key, 3, "", 0,
                                      hex_out, sizeof(hex_out));
    ASSERT_EQ(rc, 0);
...
{
    char hex_out[65];
    ASSERT_EQ(health_check_hmac_sha256(NULL, 0, "d", 1,
                                       hex_out, sizeof(hex_out)), -1);
    ASSERT_EQ(health_check_hmac_sha256("k", 1, NULL, 0,
                                       hex_out, sizeof(hex_out)), -1);
    ASSERT_EQ(health_check_hmac_sha256("k", 1, "d", 1, NULL, 0), -1);
    /* Buffer too small */
    ASSERT_EQ(health_check_hmac_sha256("k", 1, "d", 1,
                                       hex_out, 32), -1);
    return 0;
}
...
static int test_hmac_sha256_rfc4231_case3(void)
{
    ...
    char hex_out[65];
    int rc = health_check_hmac_sha256(key, 20, data, 50,
                                      hex_out, sizeof(hex_out));
    ASSERT_EQ(rc, 0);
    ASSERT_STR_EQ(hex_out, expected);
    return 0;
...
static int test_token_generate_validate_admin(void)
{
    ...
    int rc = health_check_generate_token(secret, HC_TOKEN_SCOPE_ADMIN,
                                         token, sizeof(token));
    ASSERT_EQ(rc, 0);
    ASSERT_TRUE(strlen(token) > 0);

    hc_auth_result_t result =
        health_check_validate_token(token, secret, 3600);
...
static int test_token_generate_validate_readonly(void)
{
    ...
    int rc = health_check_generate_token(secret, HC_TOKEN_SCOPE_READONLY,
                                         token, sizeof(token));
    ASSERT_EQ(rc, 0);
...
{
    char token[256];
    ASSERT_EQ(health_check_generate_token(NULL, HC_TOKEN_SCOPE_ADMIN,
                                          token, sizeof(token)), -1);
    ASSERT_EQ(health_check_generate_token("secret", HC_TOKEN_SCOPE_ADMIN,
                                          NULL, 0), -1);
    ASSERT_EQ(health_check_generate_token("secret", HC_TOKEN_SCOPE_ADMIN,
                                          token, 10), -1); /* too small */
    return 0;
}
...
static int test_token_expiry_expired(void)
{
    ...
    char hmac_hex[65];
    health_check_hmac_sha256(secret, strlen(secret),
                             message, strlen(message),
                             hmac_hex, sizeof(hmac_hex));

    char token[256];
    snprintf(token, sizeof(token), "%ld.admin.%s",
             (long)old_time, hmac_hex);
...
static int test_token_expiry_no_check(void)
{
    ...
    hc_auth_result_t result =
        health_check_validate_token(token, secret, 0);
    ASSERT_EQ(result, HC_AUTH_OK_ADMIN);
...
static int test_token_scope_admin(void)
{
    ...
    health_check_generate_token(secret, HC_TOKEN_SCOPE_ADMIN,
                                token, sizeof(token));
    ASSERT_EQ(health_check_validate_token(token, secret, 3600),
              HC_AUTH_OK_ADMIN);
...
static int test_token_scope_readonly(void)
{
    ...
    health_check_generate_token(secret, HC_TOKEN_SCOPE_READONLY,
                                token, sizeof(token));
    ASSERT_EQ(health_check_validate_token(token, secret, 3600),
              HC_AUTH_OK_READONLY);
...
{
    char token[256];
    health_check_generate_token("correct-secret", HC_TOKEN_SCOPE_ADMIN,
                                token, sizeof(token));

    ASSERT_EQ(health_check_validate_token(token, "wrong-secret", 3600),
              HC_AUTH_DENIED);
    return 0;
}
...
static int test_token_invalid_format(void)
{
    ...
    ASSERT_EQ(health_check_validate_token("nodots", "s", 0),
              HC_AUTH_DENIED);
    ASSERT_EQ(health_check_validate_token("one.dot", "s", 0),
              HC_AUTH_DENIED);
    ASSERT_EQ(health_check_validate_token("nan.admin.abc", "s", 0),
              HC_AUTH_DENIED);
    ASSERT_EQ(health_check_validate_token("123.badscope.abc", "s", 0),
              HC_AUTH_DENIED);
    /* HMAC too short */
    ASSERT_EQ(health_check_validate_token("123.admin.short", "s", 0),
              HC_AUTH_DENIED);
    return 0;
}
...
{
    const char *raw =
        "DELETE /api/v1/sessions/123 HTTP/1.1\r\n"
        "\r\n";
    hc_http_request_t req;
